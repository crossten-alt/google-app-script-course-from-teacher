...
function onOpen() {
  SpreadsheetApp.getUi()
    ...
    .addItem("📊 彙整所有週報", "彙整週報")
    .addItem("📧 發送未交週報提醒", "發送未交週報提醒") 
    .addSeparator()
    .addItem("⏰ 設定週一自動建立", "設定週一自動建立")
    .addItem("⏰ 設定週五自動彙整", "設定週五自動彙整")
    .addItem("📧 設定週五自動催繳", "設定週五自動催繳")

    .addToUi();
}

/**
 * 自動檢查尚未填寫週報的部門，並發送 Email 提醒
 */
function 發送未交週報提醒() {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var sheets = ss.getSheets();
  
  // 📝 【請修改這裡】：將後面的信箱改成你們公司真實的主管信箱
  // 如果你在測試階段，可以全部填入你自己的信箱來測試！
  var 部門信箱 = {
    "業務部": "crossten@gmail.com",
    "行銷部": "crossten@gmail.com",
    "研發部": "crossten@gmail.com",
    "人資部": "crossten@gmail.com",
    "財務部": "crossten@gmail.com"
  };
  ...
  sheets.forEach(function(sheet) {
    ...
    var 部門名 = sheet.getName().replace("週報_", "").split("_")[0];
        ...
        // 分別讀取「本週完成事項」(7~12列) 與「下週預計工作」(16~21列)
    var 完成資料 = sheet.getRange(7, 1, 6, 5).getValues(); 
    var 預計資料 = sheet.getRange(16, 1, 6, 5).getValues(); 
    var 有完成資料 = false;
    var 有預計資料 = false;

    // 檢查本週是否填寫
    完成資料.forEach(function(row) {
      if (row[1] && String(row[1]).trim() !== "") 有完成資料 = true;
    });

    // 檢查下週是否填寫
    預計資料.forEach(function(row) {
      if (row[1] && String(row[1]).trim() !== "") 有預計資料 = true;
    });

    // 只要「本週沒寫」或者是「下週沒寫」，就判定為不合格（未交）
    if (!有完成資料 || !有預計資料) {
      ...
      var email = 部門信箱[部門名];
      if (email) {
        var subject = "【系統提醒】" + 部門名 + " 尚未填寫本週週報！";
        var body = "各主管您好，\n\n系統偵測到「" + 部門名 + "」尚未完整填寫『本週完成事項』或『下週預計工作』。\n請盡速點擊下方連結前往填寫，以免影響下午的彙整作業：\n\n" + ss.getUrl() + "\n\n(此為系統自動發送，請勿直接回覆)";

        
        try {
          // 呼叫 Gmail 服務寄信
          MailApp.sendEmail(email, subject, body);
          Logger.log("✅ 已發送提醒信給：" + 部門名);
        } catch(e) {
          Logger.log("❌ 發信失敗 (" + 部門名 + ")：" + e.message);
        }
      }
    }
  });
  ...
  // 跳出提示視窗告訴你結果
  if (未交部門清單.length > 0) {
    SpreadsheetApp.getUi().alert("📧 已經發送催繳信給以下部門：\n" + 未交部門清單.join("、"));
  } else {
    SpreadsheetApp.getUi().alert("🌟 太棒了！所有部門都已經完成週報了。");
  }
}
...
function 設定週五自動催繳() {
  ScriptApp.getProjectTriggers().forEach(function(t) {
    if (t.getHandlerFunction() === "發送未交週報提醒") ScriptApp.deleteTrigger(t);
  });

  ScriptApp.newTrigger("發送未交週報提醒")
    .timeBased()
    .onWeekDay(ScriptApp.WeekDay.FRIDAY)
    .atHour(15) // 設定下午 3 點檢查
    .create();

  SpreadsheetApp.getUi().alert("✅ 每週五 15:00 自動催繳已設定！");
}
