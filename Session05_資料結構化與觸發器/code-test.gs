...
function 函數進階示範() {
  // --- 回傳物件 ---
  var 員工資訊 = 取得員工統計();
  console.log("總人數：", 員工資訊.總人數);
  console.log("平均薪資：", 員工資訊.平均薪資);
  console.log("最高薪資：", 員工資訊.最高薪資);
  ...
  console.log("大於 3 的數字：", 篩選結果); // [4, 5, 9, 6]
}
...
function 資料結構化示範() {
  ...
  for (var k = 0; k < 員工列表.length; k++) {
    console.log(員工列表[k]["姓名"] + " - " + 員工列表[k]["部門"] + " - NT$" + 員工列表[k]["月薪"]);
  }
  ...
  console.log("\n===== 部門分組 =====");
  for (var 部門 in 部門分組) {
    console.log(部門 + "：" + 部門分組[部門].length + " 人");
    for (var m = 0; m < 部門分組[部門].length; m++) {
      console.log("  - " + 部門分組[部門][m]["姓名"]);
    }
  }
}
...
function 定時更新專案狀態() {
  try {
    ...
    console.log("✅ 專案狀態已更新（" + 更新結果.length + " 筆）");

  } catch (錯誤) {
    console.error("❌ 定時更新錯誤：", 錯誤.message);
  }
}
...
function 生成優先級摘要() {
  try {
    ...
    // 💡 步驟 2：使用前面學過的「依欄位分組」工具
    var 優先級分組 = 依欄位分組(專案列表, "優先級");

    var 總專案 = 專案列表.length;
    var 摘要 = "📊 專案優先級摘要\n\n總專案數：" + 總專案 + "\n";

    // 💡 步驟 3：把分組好的資料，一個一個拿出來組裝文字
    var 優先順序 = [
      { 級別: "高", 符號: "🔥" },
      { 級別: "中", 符號: "⚠️" },
      { 級別: "低", 符號: "☕" }
    ...
    for (var k = 0; k < 優先順序.length; k++) {
      var 級別 = 優先順序[k].級別;
      var 符號 = 優先順序[k].符號;
      
      // 把該級別的陣列拿出來 (如果沒有半個，就給空陣列 [])
      var 該級別專案 = 優先級分組[級別] || []; 
      
      摘要 += "\n" + 符號 + " " + 級別 + "優先級 (" + 該級別專案.length + " 個)：\n";
...
function 生成薪資報表() {
  try {
    ...
    // 💡 步驟 3：準備要寫入試算表的「二維陣列」 (第一列先放標題)
    var 報表資料 = [["部門", "總人數", "平均薪資"]]; 
    
    for (var 部門名 in 部門分組) {
      var 成員 = 部門分組[部門名]; // 拿到該部門的所有員工
      var 總人數 = 成員.length;
      
      // 拿出第一部分學到的 reduce 技巧來算總和！
      var 薪資總和 = 成員.reduce(function(sum, val) { 
        return sum + Number(val["月薪"]); 
      }, 0);
      
      var 平均薪資 = Math.round(薪資總和 / 總人數);
      
      // 把算好的這三個數字，打包成一個小陣列，推入報表資料中
      報表資料.push([部門名, 總人數, 平均薪資]);
    }

    // 💡 步驟 4：建立並寫入工作表
    var 報表 = ss.getSheetByName("薪資報表");
    if (報表) {
      報表.clear(); // 如果存在就清空
    } else {
      報表 = ss.insertSheet("薪資報表"); // 如果不存在就建立
    }

    // 一次性批次寫入全部資料 (最注重效能的寫法！)
    報表.getRange(1, 1, 報表資料.length, 3).setValues(報表資料);
    
    // 💡 步驟 5：幫報表「化妝」排版
    報表.getRange(1, 1, 1, 3).setBackground("#4caf50").setFontColor("#fff").setFontWeight("bold");
    報表.getRange(2, 3, 報表資料.length - 1, 1).setNumberFormat("#,##0"); // 加上千分位
    報表.setFrozenRows(1); // 凍結第一列
    for (var c = 1; c <= 3; c++) {
      報表.autoResizeColumn(c); // 自動調整欄寬
    }
