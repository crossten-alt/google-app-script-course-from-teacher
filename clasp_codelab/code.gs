...
function 一鍵產生報表簡報() {
  try {
    ...
    var table = KPI頁.insertTable(表格列數, 4, 40, 270, 640, 100);
    ...
    var htmlContent = 
      '<div style="font-family: \'Microsoft JhengHei\', Arial, sans-serif; padding: 15px; text-align: center;">' +
      '  <h3 style="color: #1a237e; margin-top: 0; font-size: 18px;">🎉 報表簡報已成功產生！</h3>' +
      '  <p style="font-size: 14px; color: #333; margin: 8px 0;">📄 頁數：' + ppt.getSlides().length + ' 頁 ｜ ⏱️ 耗時：' + 耗時 + ' 秒</p>' +
      '  <p style="font-size: 12px; color: #666; margin-bottom: 20px; word-break: break-all;">📎 名稱：' + 簡報名 + '</p>' +
      '  <a href="' + ppt.getUrl() + '" target="_blank" style="' +
      '    display: inline-block; padding: 10px 24px; background-color: #1a73e8; ' +
    ...
    var htmlOutput = HtmlService.createHtmlOutput(htmlContent)
      .setWidth(400)
      .setHeight(200);
    SpreadsheetApp.getUi().showModelessDialog(htmlOutput, "系統提示");
